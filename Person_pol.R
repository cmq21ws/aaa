...
library(tidyverse)
 ...
 Person_pol= R6Class("Person_pol",
                 public = list(
                   #The lines below 'initialise' or specify the initial values for all of the simulated var-s (either by specifying manual values or refering to target data)
                   initialize = function(rdf, mortality_probs, life_expectancy, i, 
                     ...
                     #Mortality each year by sex and IMD - target data
                     private$mortality_probs = mortality_probs
                     private$life_expectancy = life_expectancy
                     #Targetdata for various health related outcomes by sex, age, and IMD quintile 
                     private$targetdata_for_mort = targetdata_for_mort
                     #Targetdata and trends for various outcomes by sex, age  
                     private$targetdata_sexage = targetdata_sexage
                     #Targetdata for various outcomes by sex, age  
                     ...
                     private$life_history = tibble(male = if(rdf$sex[i]==1) ('TRUE') else('FALSE'),
                                          ...
                                          effect=0,
                                          effect2=0,
                                          #SDQ conduct problem subscore sweep 3 (standardised)
                                          social_emotional_capital3 = (10-rdf$sdq_cond3[i])/10,
                                          #SDQ conduct problem subscore sweep 4 (standardised)
                                          social_emotional_capital4 = (10-rdf$sdq_cond4[i])/10,
                                          #SDQ conduct problem subscore sweep 5 (standardised)
                                          social_emotional_capital5 = (10-rdf$sdq_cond5[i])/10,
                                          #SDQ conduct problem subscore sweep 6 (standardised)
                                          social_emotional_capital6 = (10-rdf$sdq_cond6[i])/10,
                                          #SDQ impact score sweep 2 (standardised)
                                          impact2 =  rdf$impact2[i],
                                          #SDQ impact score sweep 3 (standardised)
                                          impact3 =  rdf$impact3[i],
                                          #SDQ impact score sweep 4 (standardised)
                                          impact4 = rdf$impact4[i],
                                          impact=0,
                                          #Standard deviation of SDQ conduct problem subscore at age 8 (sweep 4)
                                          sd_soc8=((filter(average_data_total,AGE==8, SEX==1)$sd_soc)*(filter(average_data_total,AGE==8, SEX==1)$num)+(filter(average_data_total,AGE==8, SEX==0)$sd_soc)*(filter(average_data_total,AGE==8, SEX==0)$num))/(filter(average_data_total,AGE==8, SEX==1)$num+filter(average_data_total,AGE==8, SEX==0)$num),
                                          #Standard deviation of SDQ impact score at age 8 (sweep 4)
                                           sd_imp8=((filter(average_data_total,AGE==8, SEX==1)$sd_imp)*(filter(average_data_total,AGE==8, SEX==1)$num)+(filter(average_data_total,AGE==8, SEX==0)$sd_imp)*(filter(average_data_total,AGE==8, SEX==0)$num))/(filter(average_data_total,AGE==8, SEX==1)$num+filter(average_data_total,AGE==8, SEX==0)$num),
                   ...
                   live_life = function(){
                     ...
                     current_year = previous_year
                      while(current_year$life_stage != "dead" & current_year$age < 100){
                       ...
                       #specify the constant var-s over time (i.e. make them stay constant and simply take the value of the previous period)
                       current_year$sd_soc8=previous_year$sd_soc8
                       ...
                       #Opportunity costs  (simple way to model these)  
                     ...
                     #Probability of dying from CHD  by sex and age bracket
                      ...
                      current_year$pdeath= (filter(private$mortality_probs, AGE==current_year$age, MALE==current_year$male,
                       ...
                       if(runif(1)<( (filter(private$mortality_probs, AGE==current_year$age, MALE==current_year$male,
                         ...
                         #Specify the variable dependencies for the functions driving the evolution of the outcomes (in the list 'private') -- the functions are specified further down (the list 'private)
                         current_year$life_stage = private$get_life_stage(current_year$age)
                         current_year$effect=private$get_effect(previous_year$effect, current_year$age ,current_year$sex,  current_year$recip,current_year$sd_soc8 )
                         current_year$effect2=private$get_effect2(previous_year$effect2, current_year$age ,current_year$sex,current_year$recip , current_year$sd_imp8 )
                 ...
                 private = list(
                   ...
                   no_recip=NULL,
                   no_total=NULL,
                   #Specify the diferent life stages
                   ...
                   #Specify the functions driving the evolution of the outcomes 
                   
           #Examples of policy effects on SDQ conduct and impact scores (if modelling policy) 
           #SDQ conduct problems and impact scores have a scale of 0-10, with a higher value representing more problems
           #average 0.46 standard deviation decrease in the SDQ conduct problem and impact scores
           #kessler_p is the depression score for the parents
           #sdqcond3 is the baseline conduct score for child at the start
           #sd_soc5 is the Standard deviation of SDQ conduct problem subscore at age 5
           #recip refers to the below lines of code would only apply on recipients
           #if we do not want to apply policy, we only need to multiply the value of effect by 0
#so what I would assume is that for people who have a good mental health at baseline, there is no change
#but for people who have poor mental health, they might have a big change, which wouuld based on the literature
#But SDQ in this method also have disadvantages (because they only focus on the conduct score(the behavior), not the whole parts), 
#I might want to put this in the discussion part
                  

              
 
                  #conduct scores 
                get_effect=function(previous_effect, age, sex, recip,sd_soc8){
                    #only individuals with conduct score over 4 would get a referral 
                  #and an effect and there is 29% probability that people may accept further treatment
                  if((age==8) &  runif(1)< 0.29){effect=max(0,(0.19*sd_soc8+0*rnorm(1, mean=0, sd=sd_soc8*0.46/4))*recip)
                     }else {
                       effect=previous_effect  #assumes that any effect of 8 would remain lifetime so once someone gets better, and it would stay better.
                     }   
                     return(effect)},
                   ...
                   #impact scores
                  get_effect2=function(previous_effect2, age, sex,   recip ,sd_imp8){
                  if(age==8& runif(1)< 0.29){effect2=max(0, (0.19*sd_imp8+0*rnorm(1, mean=0, sd=sd_imp8*0.46/4))*recip)
                  }else {
                  ...
                  return(effect2)},
                   ...
                   #original code of effects for reference
                   # get_effect=function(previous_effect, age, sex, kessler_p, sdqcond3, sd_soc5,  recip){
                   #if(age==5){effect=max(0, ((0.46+(3.4/37)*(kessler_p/3.45-12.2/10.5)+(1.74/37)*(sdqcond3/(sd_soc5*10)-139.4/37))*sd_soc5+0*rnorm(1, mean=0, sd=sd_soc5*0.46/4))*recip)
                   # }else {
                   ...
                   # return(effect)},
                    ...
                    #if(age==5){effect2=max(0, ((0.46+(3.4/37)*(kessler_p/3.45-12.2/10.5)+(1.74/37)*(sdqcond3/(sd_soc5*10)-139.4/37))*sd_imp5+0*rnorm(1, mean=0, sd=sd_imp5*0.46/4))*recip)
                    #}else {
                    #effect2=previous_effect2  
                    #}   
                    #return(effect2)},
                   
                    #SDQ conduct problems score standardised as 'social-emotional capital'(=1-SDQ conduct problems score/10)
                  ...
                  #These are just taken from the initialdata. So it is not doing anything or adding an effect
                   #Cognitive skills
                   ...
                   get_dcognitive_capital = function(previous_cognitive_capital, cognitive_capital){
                     ...
                     #For depression, be aware of where the data come from and what leva has done.    
                      #According to the formula in the Appendix of LifeSim model for depression, we know that depressiuon would be affected by cd(Childhood conduct disorder)
